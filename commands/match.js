...
module.exports = {
  ...
  async execute(message, args) {
    ...
    if (message.channel.id !== config.matchesChannelId) {
      return message.reply("❌ استخدم الأمر في قناة المباريات فقط.");
    }

    const input = args.join(" ").split("|").map(value => value.trim());

    if (input.length !== 3) {
      return message.reply(
        "❌ الاستخدام:\n`مباراة الفريق الأول | الفريق الثاني | 2026-07-20 21:00`"
      );
    }
    ...
    if (!homeTeam || !awayTeam || Number.isNaN(kickoffAt.getTime())) {
      return message.reply("❌ تأكد من أسماء الفرق والتاريخ.");
    }

    if (kickoffAt <= new Date()) {
      return message.reply("❌ موعد المباراة لازم يكون بالمستقبل.");
    }
    ...
    const unixTime = Math.floor(kickoffAt.getTime() / 1000);

    const embed = new EmbedBuilder()
      .setColor(0x2ecc71)
      .setTitle("⚽ مباراة جديدة")
      .setDescription(`**${homeTeam} ضد ${awayTeam}**`)
      .addFields(
        {
          name: "موعد المباراة",
          value: `<t:${unixTime}:F>\n<t:${unixTime}:R>`
